---
layout: post
title: Implementing Fast Paxos In TLA+
date: 2023-08-20 17:04:34
categories:
...
\* https://github.com/fpaxos/ffpaxos-tlaplus/blob/master/FastFlexiblePaxos.tla
\* tlatex -shade FastPaxos.tla
\* dvipdfm FastPaxos.dvi
------------------------- MODULE FastPaxos ----------------------------------
(***************************************************************************)
...
(* sets of acceptors, for every round number $i$,                          *)
(* and that the Quorum Requirement (Section 3.1, page 19) holds.           *)
(***************************************************************************)
ASSUME \A i \in RNum :
...
VARIABLES rnd, vrnd, vval,
          crnd, cval,  (****************************************************)
                       (*`^$crnd[c]$ is the highest-numbered round that    *)
                       (* $c$ has begun, initially 0.                      *)
                       (* $cval[c]$ is the value that $c$ has picked for   *)
                       (* round $crnd[c]$, or the special value $none$     *)
                       (* if $c$ has not yet picked a value for that       *)
                       (* round.^'                                         *)
          amLeader,    (****************************************************)
...
FastNum <- const_FastNum
Quorum <- const_Quorum
Coord <- const_Coord
CoordOf <- const_CoordOf
...
+ [GitHub: fpaxos/ffpaxos-tlaplus](https://github.com/fpaxos/ffpaxos-tlaplus)
