...
The way to decide which register sets are fast sets, which register sets are classic sets are a little complicated, and if you are interesting on that, you can read Fast Paper. In current blog, we only discuss the simplest way. We say register set $0$ is the only one fast set, all reset register sets are classic sets. Then a client does not need to be assigned register set $0$ nor do they need to ensure that they write to it with only one value. Clients can write many different not-nil values to different registers of register set $0$. This involve a question for register sets with larger indices (such as register set $1$). Remeber Heidi Howard said "each quorum is in one of four decision states", but each register set have multiple quorums, each quorum can in state "Maybe $v$" or "Maybe $v^\prime$", causing the decision table of correpsoning register set is "Maybe $v, v^\prime$". The Single-Decree Synod will not meet this due to it use restricted configurations to meet rule 3, however, fast paxos may meet this. for example, the question is the client writing register set $1$, it cannot know the decision state of register set $0$ as simple as the Single-Decree Synod. Because of different registers of register set $0$ can be written to different values, then the client can get different decision state based on different quorum, such as $\text{Maybe}(v)$ based on one quorum, and $\text{Maybe}(v^\prime)$ where $v^\prime \neq v$ based on another quorum. So the client is stucked, it cannot write to register set $1$ before the decision state of register set $0$ changes.
